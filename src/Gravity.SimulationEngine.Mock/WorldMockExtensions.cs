...
public static class WorldMockExtensions
{
	#region Internal types

	extension(WorldMock world)
	{
		#region Interface

		public static async Task<(IWorld World, Entity[] Entities, TimeSpan DeltaTime)> CreateFromJsonResourceAsync(string jsonResourcePath, Assembly? resourceAssembly = null)
		{
			...
			return await WorldMock.CreateFromJsonAsync(json);
		}

		public static async Task<(IWorld World, Entity[] Entities, TimeSpan DeltaTime)> CreateFromJsonAsync(string json)
		{
			...
			var ret = new WorldMock(vp, state.ClosedBoundaries, state.ElasticCollisions);
			...
			return (ret, entities, TimeSpan.FromSeconds(1.0d / 60.0d * Math.Pow(10, state.TimeScale)));
		}
