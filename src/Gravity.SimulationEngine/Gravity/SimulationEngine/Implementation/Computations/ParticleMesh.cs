		...
		Parallel.For(0, n2, slice =>
							{
								...
								var buf = new Complex[n];
								for(var j = 0; j < n; j++)
									buf[j] = data[(k * n + j) * n + ii];
								if(forward)
									Fourier.Forward(buf, FourierOptions.NoScaling);
								else
									Fourier.Inverse(buf, FourierOptions.NoScaling);
								for(var j = 0; j < n; j++)
									data[(k * n + j) * n + ii] = buf[j];
							});
		...
		Parallel.For(0, n2, slice =>
							{
								...
								var buf = new Complex[n];
								for(var k = 0; k < n; k++)
									buf[k] = data[(k * n + jj) * n + ii];
								if(forward)
									Fourier.Forward(buf, FourierOptions.NoScaling);
								else
									Fourier.Inverse(buf, FourierOptions.NoScaling);
								for(var k = 0; k < n; k++)
									data[(k * n + jj) * n + ii] = buf[k];
							});
							   ...
							   for(var jj = 0; jj < n; jj++)
							   {
								   ...
								   for(var ii = 0; ii < n; ii++)
								   {
									   ...
									   if(k2 > _eps)
									   {
										   ...
										   axK[idx] = new(-coeff * kxVal * rhoIm, coeff * kxVal * rhoRe);
										   ayK[idx] = new(-coeff * kyVal * rhoIm, coeff * kyVal * rhoRe);
										   azK[idx] = new(-coeff * kzVal * rhoIm, coeff * kzVal * rhoRe);
									   }
									   else
		...
		Parallel.For(0, nBodies, bi =>
								 {
									 ...
									 b.a = new(w000 * axK[idx].Real + w100 * axK[idx + 1].Real +
