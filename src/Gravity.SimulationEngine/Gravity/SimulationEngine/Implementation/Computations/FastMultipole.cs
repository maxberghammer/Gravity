...
internal sealed class FastMultipole : SimulationEngine.IComputation
{
	...
	private sealed class Cell
	{
		...
		public double Qxx,
					  Qyy,
					  Qzz; // Quadrupole diagonal

		public double Qxy,
					  Qxz,
					  Qyz; // Quadrupole off-diagonal
	...
	private static Cell BuildOctree(IReadOnlyList<Body> bodies)
	{
		...
		var center = new Vector3D((minX + maxX) * 0.5,
								  (minY + maxY) * 0.5,
								  (minZ + maxZ) * 0.5);
	...
	private static void Subdivide(Cell cell, IReadOnlyList<Body> bodies)
	{
		if(cell.BodyIndices == null ||
		   cell.BodyIndices.Count <= _maxBodiesPerLeaf ||
		   cell.Depth >= _maxDepth)
			return;
	...
	private static void ComputeLeafMultipole(Cell cell, IReadOnlyList<Body> bodies)
	{
		...
		cell.Mass = totalMass;
		cell.CenterOfMass = new(comX / totalMass, comY / totalMass, comZ / totalMass);
